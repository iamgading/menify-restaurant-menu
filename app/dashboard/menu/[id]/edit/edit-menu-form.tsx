...
import { Utensils, DollarSign, AlignLeft, List, CheckCircle2, Loader2 } from "lucide-react"
...
export function EditMenuForm({ menuItem, categories }: EditMenuFormProps) {
  // @ts-ignore
    ...
    <form action={formAction} className="space-y-8">
      <div className="grid gap-6">
        ...
        <div className="space-y-3">
          ...
          {/* Current Image */}
          {menuItem.image_url && !imagePreview && (
            <div className="mb-3">
              <p className="text-sm text-muted-foreground mb-2">Gambar saat ini:</p>
              <img 
                src={menuItem.image_url} 
                alt={menuItem.name}
                className="w-32 h-32 object-cover rounded-lg border-2 border-stone-200 dark:border-stone-800"
              />
            </div>
          )}
          ...
          {imagePreview && (
            <div className="relative w-full max-w-xs mb-3">
              <p className="text-sm font-medium text-orange-600 dark:text-orange-400 mb-2">Gambar baru (akan mengganti gambar lama):</p>
              <img 
                src={imagePreview} 
                alt="Preview"
              ...
              <p className="text-xs text-green-600 dark:text-green-400 mt-2 font-medium">✓ Gambar baru siap di-upload</p>
            </div>
          )}
              ...
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  reader.onloadend = () => {
                    ...
                    toast.success('Gambar baru berhasil dipilih!')
                  }
          ...
          <p className="text-xs text-muted-foreground">Format: JPG, PNG, atau WebP. Maksimal 5MB. Kosongkan jika tidak ingin mengubah gambar.</p>
        </div>
        ...
        <div className="flex items-center justify-between p-4 rounded-xl bg-stone-50 dark:bg-stone-900/50 border border-stone-100 dark:border-stone-800">
          ...
          <div className="flex items-center gap-2">
            <input 
              ...
              id="is_available" 
              name="is_available" 
              defaultChecked={menuItem.is_available}
